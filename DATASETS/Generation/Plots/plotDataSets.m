...
t2 = readtable('2ndSCSDM_DataSet_longrun1.csv');
data2 = table2array(t2);

t3 = readtable('3or21CascadeSDM_DataSet_longrun1.csv');
data3 = table2array(t3);

t4 = readtable('211CascadeSDM_DataSet_longrun1.csv');
...
legendLabels = {'2orSCSDM Single Bit', '3orCascadeSDM', '211CascadeSDM'}; % Add your custom labels here

for i = 1:length(uniqueLabels)
    currentClass = uniqueLabels(i);
    indices = (ds(:, 4) == i - 1);

    scatter3(ds(indices, 1), ds(indices, 2), ds(indices, 3), 20, ds(indices, 4), 'filled', 'DisplayName', legendLabels{i}, 'MarkerFaceAlpha', 0.6);
    hold on;
end
